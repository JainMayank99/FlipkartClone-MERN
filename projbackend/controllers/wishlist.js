...
exports.addProductToWishList = async (req, res) => {
    ...
    await WishListSchema.find({ user: req.profile._id, product: req.product._id })
        .exec(async (err, wishlist) => {
            ...
            if (wishlist.length != 0) {
                return res.status(400).json({
                    err: 'Item already exists in wishlist in DB !'
                });
            }
...
exports.removeProductFromWishList = async (req, res) => {
    await WishListSchema.findOneAndDelete({ user: req.profile._id, product: req.product._id })
        .exec((err, wishlist) => {
            if (err || !wishlist) {
                return res.status(400).json({
